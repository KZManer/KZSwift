...
- (void)progressSliderTouchChanged:(JJSlider *)slider{
    NSLog(@"touch changed");
    if (self.delegate && [self.delegate respondsToSelector:@selector(jj_playerMaskViewProgressSliderTouchChanged:)]) {
        [self.delegate jj_playerMaskViewProgressSliderTouchChanged:slider];
    }else{
